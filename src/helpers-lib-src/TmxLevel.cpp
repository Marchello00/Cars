...
bool TmxLevel::loadFromFile(const std::string &filepath) {
    ...
    m_spacing = std::stoi(tilesetElement->Attribute("spacing"));
    ...
    const int columns = m_tilesetImage.getSize().x / (m_tileWidth + m_spacing);
    const int rows = m_tilesetImage.getSize().y / (m_tileHeight + m_spacing);
...
void TmxLevel::render(sf::RenderTarget &target) const {
    const sf::FloatRect viewportRect = target.getView().getViewport();

    // draw all tiles (and don't render objects)
...
unsigned CSVReader::getNext() {
    if (ptr >= text.size()) return 0;
    while (text[ptr] == '\n' || text[ptr] == ',') ptr++;
    int len = 0;
    while (ptr + len < text.size() && text[ptr + len] != ',') len++;
...
void RotatorAdapter::mult(int m[2][2]) {
    int res[2][2] = {{0, 0}, {0, 0}};
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 2; ++j) {
            for (int k = 0; k < 2; ++k) {
...
void RotatorAdapter::flipVertically() {
    mult(vertical);
}

void RotatorAdapter::flipHorizontally() {
    mult(horizontal);
}

void RotatorAdapter::flipDiagonally() {
    mult(diagonal);
}
